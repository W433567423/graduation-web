...
<template>
  ...
  <Modal v-model:visible="renameDialogFormVisible" title="重命名项目" :on-before-ok="renameProject"
    @cancel="renameDialogFormVisible = false">
    <a-form ref="formRef" :model="form" :rules="formRules">
      <a-form-item validate-trigger="blur" label="新名称" required field="newName">
        <a-input v-model="form.newName" clearable placeholder="请输入新项目名" autocomplete="off" />
      </a-form-item>
    </a-form>
  </Modal>
</template> 
...
import { Modal, Notification, type FieldRule, type FormInstance, type TableColumnData } from '@arco-design/web-vue';
...
const columns: TableColumnData[] = [
  ...
  { title: '上一次运行时间', dataIndex: 'updateTime' },
  { title: '上一次运行状态', dataIndex: 'lastStatus' },
...
const renameProject = async () => {
  ...
  return new Promise(resolve => {
    formRef.value!.validateField('newName',
      async (errors) => {
        if (!errors) {
          console.log(localProject.value?.id!, form.value.newName);

          await putReNameProject(localProject.value?.id!, form.value.newName)
